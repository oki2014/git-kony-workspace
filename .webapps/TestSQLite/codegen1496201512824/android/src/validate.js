...
function createTable() {
    kony.db.transaction(BASEOBJECTID, createTableProcess);
}
...
function insertData() {
    kony.db.transaction(BASEOBJECTID, insertDataProcess, commonErrorCallback, commonVoidcallback);
}
...
function retrieveData() {
    kony.db.transaction(BASEOBJECTID, retrieveDataProcess, commonErrorCallback, commonVoidcallback);
}
...
function retrieveDataProcess(dbId) {
    ...
    kony.db.executeSql(dbId, sqlStatement, null, success_selectData, commonErrorCallback);
}

function success_selectData(transactionId, resultset) {
    var basicConf2 = {
        message: "Error." + transactionId + " | " + resultset,
    ...
    if (resultset !== null) {
        ...
        if (numResults > 0) {
            ...
            var basicConf = {
                message: "Welcome " + GLOBALUSERNAME + "! You are now logged in.",
                alertType: constants.ALERT_TYPE_INFO,
                alertTitle: "Success",
                yesLabel: "ok",
                alertHandler: alertCallBack()
